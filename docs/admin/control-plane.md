...
Perongen loads six documents at one Git revision. This makes configuration reviewable, recoverable, and portable while keeping secrets outside the repository.
...
- **Scorecards** — rules, weights, severity, and live previews;
...
- **Integrations** — deployment and delivery health;
...
All six files must validate at the same revision. Perongen keeps serving the last-known-good revision during a GitHub outage or invalid commit, but configuration writes remain disabled until synchronization recovers.
