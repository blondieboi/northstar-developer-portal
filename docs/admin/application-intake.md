...
The scan is bounded to five repositories at a time. Active, non-fork repositories are ordered by their latest push so the queue starts with software most likely to be shipping now; forks and archived repositories remain visible but appear later. Select **Scan again** after installing the GitHub App on another repository or merging an onboarding pull request.
...
Perongen may suggest a risk value from deployment, persistence, or authentication dependencies, but it deliberately leaves all three risk controls unselected. An administrator must confirm exposure, data sensitivity, and authentication before valid YAML is generated. An accountable owner is also required.
