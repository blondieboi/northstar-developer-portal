...
export const ruleSchema=z.object({id:z.string().min(1),title:z.string().min(1),description:z.string().default(''),path:z.string().min(1),operator:z.enum(['present','equals','oneOf','minLength','contains']),value:z.any().optional(),weight:z.number().positive().default(1),severity:z.enum(['required','recommended']).default('recommended'),enabled:z.boolean().default(true),tiers:z.array(slug).min(1).optional(),types:z.array(slug).min(1).optional(),source:ruleSourceSchema.optional(),remediation:remediationSchema.optional()}).strict()
export const scorecardSchema=z.object({id:slug,title:z.string().min(1),description:z.string().default(''),enabled:z.boolean().default(true),primary:z.boolean().default(false),rules:z.array(ruleSchema)}).strict()
...
export function scoreWithConfig(metadata:any,plugins:Record<string,unknown>={}){const primary=effective.scorecards.cards.find(card=>card.primary)!;return calculateScore(metadata,primary.rules,plugins)}
export function scoresWithConfig(metadata:any,plugins:Record<string,unknown>={}){return calculateScorecards(metadata,effective.scorecards.cards,plugins)}
