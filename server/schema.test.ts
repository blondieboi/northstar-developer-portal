...
describe('database schema',()=>{
  ...
  it('stores teams, sync runs, and action runs',async()=>{
    ...
    const sql=await readFile(fileURLToPath(new URL('./schema.sql',import.meta.url)),'utf8')
    db.public.none(sql)
    for(const table of ['teams','users','team_members','sync_runs','action_runs','config_state','config_sync_events','plugin_snapshots'])expect(db.public.getTable(table)).toBeTruthy()
