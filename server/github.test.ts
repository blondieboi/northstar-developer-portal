...
const complete = {
  ...
  metadata: {
    name: "checkout-api",
    description: "Coordinates checkout and payments.",
  },
  spec: {
    owner: "team:checkout",
    ...
    language: "TypeScript",
    links: [
      { name: "Documentation", url: "https://docs.example.com/checkout" },
    ],
  },
};

describe("service metadata", () => {
  it("accepts the documented metadata contract", () =>
    expect(metadataSchema.parse(complete)).toEqual(complete));
  it("accepts dependency, resource, documentation, and operational context", () => {
    const parsed = metadataSchema.parse({
      ...complete,
      spec: {
        ...complete.spec,
        dependsOn: ["service:inventory"],
        providesApis: ["checkout"],
        consumesApis: ["api:pricing"],
        resources: [
          { name: "checkout-db", type: "database", relation: "reads-writes" },
        ],
        docsPath: "engineering/docs",
        operational: {
          onCall: "payments-primary",
          runbookUrl: "https://docs.example.com/runbook",
          sloUrl: "https://grafana.example.com/slo",
        },
      },
    });
    expect(parsed.spec.dependsOn).toEqual(["service:inventory"]);
    expect(parsed.spec.resources?.[0]).toMatchObject({
      name: "checkout-db",
      type: "database",
    });
    expect(parsed.spec.operational?.onCall).toBe("payments-primary");
  });
  ...
  it("accepts lifecycle strings for runtime configuration validation", () =>
    expect(
      metadataSchema.parse({
        ...complete,
        spec: { ...complete.spec, lifecycle: "staging" },
      }).spec.lifecycle,
    ).toBe("staging"));
  it("validates a service tier against catalog configuration", () => {
    ...
    expect(() =>
      validateServiceMetadata({
        ...complete,
        spec: { ...complete.spec, tier: "urgent" },
      }),
    ).toThrow("Unsupported tier: urgent");
  });
  it("keeps tier optional for existing service metadata", () => {
    const { tier: _, ...spec } = complete.spec;
    expect(
      validateServiceMetadata({ ...complete, spec }).spec.tier,
    ).toBeUndefined();
  });
  it("validates a service type against catalog configuration", () => {
    ...
    expect(() =>
      validateServiceMetadata({
        ...complete,
        spec: { ...complete.spec, type: "worker" },
      }),
    ).toThrow("Unsupported service type: worker");
  });
  ...
  it("scores each missing recommended field independently", () => {
    const sparse = metadataSchema.parse({
      ...complete,
      metadata: { ...complete.metadata, description: "" },
      spec: { owner: "team:checkout", lifecycle: "production" },
    });
  ...
  it("accepts GitHub usernames as authoritative members", () => {
    const team = teamSchema.parse({
      ...
      spec: {
        members: ["octocat"],
        links: [
          { name: "Jira board", url: "https://jira.example.com/platform" },
        ],
      },
    });
