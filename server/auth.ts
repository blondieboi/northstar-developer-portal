...
const secret=()=>process.env.SESSION_SECRET||process.env.GITHUB_CLIENT_SECRET||'perongen-development-only'
...
export async function currentUser(request:FastifyRequest){const session=decode<SessionUser>(cookie(request,'perongen_session'));if(!session)return null;const stored=await findUser(session.login);return{...session,role:isAdminLogin(session.login)?'admin':'member',primaryTeam:stored?.primary_team as string|null}}
...
export function beginLogin(reply:FastifyReply){
  ...
  reply.header('set-cookie',`perongen_oauth_state=${encode({state,created:Date.now()})}; ${cookieFlags()}`)
...
export async function finishLogin(request:FastifyRequest<{Querystring:{code?:string;state?:string;installation_id?:string}}>,reply:FastifyReply){
  const saved=decode<{state:string;created:number}>(cookie(request,'perongen_oauth_state'))
  ...
  const userResponse=await fetch('https://api.github.com/user',{headers:{authorization:`Bearer ${token.access_token}`,accept:'application/vnd.github+json','user-agent':'perongen-portal'}})
  ...
  reply.header('set-cookie',`perongen_session=${encode(user)}; ${cookieFlags()}`)
  return reply.redirect(frontendUrl())
}

export function logout(reply:FastifyReply){reply.header('set-cookie','perongen_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0');return {status:'signed_out'}}
