...
await initializeGitConfig(async(changed,config)=>{
  if(changed.includes('scorecards'))await recalculateScores(scoreWithConfig)
...
server.get('/api/portal',async()=>({general:getConfig().general,scorecards:getConfig().scorecards,actions:getConfig().actions.definitions.filter(a=>a.enabled&&a.published),tools:getConfig().tools}))
