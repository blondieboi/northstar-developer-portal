...
import { catalogSummary, listAuditEvents, listServices, listTeams, listUsers, listWebhooks, migrate, onboardingStats, recordAction, recordSync, recordWebhook, setUserPrimaryTeam, upsertUser, webhookSeen } from './db.js'
...
import { defaults, getBreakGlassAdmins, getConfig, isAdminLogin, type ConfigSection } from './config.js'
import { metadataChanged, pluginRefreshRequested, verifyWebhookSignature } from './webhook.js'
...
import { decorateServicesWithPlugins, pluginCatalogResponse, pluginFactsForService, recalculateAllServiceScorecards, refreshAllServicePlugins, refreshRepositoryPlugins } from './plugins/runtime.js'
...
await initializeGitConfig(async(changed,config)=>{
  if(changed.includes('scorecards')||changed.includes('catalog'))await recalculateAllServiceScorecards()
  if(changed.includes('integrations'))queueMicrotask(()=>{refreshAllServicePlugins().catch(error=>server.log.error(error))})
...
server.get('/api/services',async()=>({services:await decorateServicesWithPlugins((await listServices())||[])}))
server.get('/api/teams',async()=>({teams:await listTeams()}))
server.get('/api/users',async()=>({users:await listUsers()}))
server.get('/api/summary',async()=>{const summary=await catalogSummary();return{...summary,services:await decorateServicesWithPlugins(summary.services||[])}})
server.get('/api/portal',async()=>({general:getConfig().general,catalog:{tiers:getConfig().catalog.tiers,types:getConfig().catalog.types},scorecards:getConfig().scorecards,integrations:{plugins:await pluginCatalogResponse()},actions:getConfig().actions.definitions.filter(a=>a.enabled&&a.published),tools:getConfig().tools}))
server.get('/api/plugins',async()=>({plugins:await pluginCatalogResponse()}))
server.get<{Params:{serviceName:string}}>('/api/plugins/services/:serviceName',async request=>({plugins:await pluginFactsForService(request.params.serviceName)}))
server.post('/api/admin/plugins/refresh',{preHandler:requireAdmin},async()=>{await refreshAllServicePlugins();return{status:'refreshed'}})
...
server.post('/api/github/webhook',async(request,reply)=>{
  ...
  try{
    ...
    if(pluginRefreshRequested(event,repository)){await refreshRepositoryPlugins(repository);await recordWebhook({deliveryId,event,action:'plugin.refresh',repository,installationId,status:'applied',message:'GitHub Actions data refreshed'});return reply.code(202).send({status:'applied'})}
