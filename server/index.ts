...
server.get('/api/portal',async()=>({general:getConfig().general,catalog:{tiers:getConfig().catalog.tiers,types:getConfig().catalog.types},scorecards:getConfig().scorecards,actions:getConfig().actions.definitions.filter(a=>a.enabled&&a.published),tools:getConfig().tools}))
